...
def can_build():
    if os.name == "nt":
        ...
        # If VCINSTALLDIR is set in the OS environ, use traditional Nebula logic to set up MSVC
...
def get_opts():
    ...
    if d3d12_deps_folder:
        d3d12_deps_folder = os.path.join(d3d12_deps_folder, "Nebula", "build_deps")
    else:
...
def setup_msvc_auto(env: "SConsEnvironment"):
    ...
    # aliases or aren't supported by Nebula (itanium & ia64).
...
def configure_msvc(env: "SConsEnvironment", vcvars_msvc_config):
    ...
    if env["d3d12"]:
        ...
        if not os.path.exists(env["mesa_libs"]):
            print_error(
                ...
                "https://docs.nebulaengine.org/en/latest/contributing/development/compiling/compiling_for_windows.html"
            )
            sys.exit(255)
    ...
    env.Append(LINKFLAGS=["/NATVIS:platform\\windows\\nebula.natvis"])
