...
var project_name: String:
	set(new_name):
		if new_name.length() == 0:
			...
			project_path = new_name.to_snake_case().replace("_","-")
		project_name = new_name

var project_path: String:
	set(new_path):
		if new_path and !custom_project_path:
			project_path = EngineConfig.default_project_path.path_join(new_path) + '/'
		elif custom_project_path:
			project_path = custom_project_path.path_join(new_path) + '/'
		else:
			project_path = EngineConfig.default_project_path.path_join("my-project/")
