...
using NebulaTools.Internals;
using Directory = NebulaTools.Utils.Directory;
using File = NebulaTools.Utils.File;
using OS = NebulaTools.Utils.OS;
using Path = System.IO.Path;
using System.Globalization;

namespace NebulaTools.Export
{
    public partial class ExportPlugin : EditorExportPlugin
    {
        ...
        {
            return File.Exists(NebulaSharpDirs.ProjectSlnPath);
        }
        ...
        public override Nebula.Collections.Array<Nebula.Collections.Dictionary> _GetExportOptions(EditorExportPlatform platform)
        {
            return new Nebula.Collections.Array<Nebula.Collections.Dictionary>()
            {
                new Nebula.Collections.Dictionary()
                {
                    {
                        "option", new Nebula.Collections.Dictionary()
                        {
        ...
        public override void _ExportFile(string path, string type, string[] features)
        {
            ...
            if (!ProjectContainsDotNet())
            {
                _maybeLastExportError = $"This project contains C# files but no solution file was found at the following path: {NebulaSharpDirs.ProjectSlnPath}\n" +
            ...
            if (!includeScriptsContent)
            {
                ...
                // Sadly, Nebula prints errors when adding an empty file (nothing goes wrong, it's just noise).
                // Because of this, we add a file which contains a line break.
                AddFile(path, System.Text.Encoding.UTF8.GetBytes("\n"), remap: false);

                // Tell the Nebula exporter that we already took care of the file.
                Skip();
            }
        }
        ...
        private void _ExportBeginImpl(string[] features, bool isDebug, string path, long flags)
        {
            ...
            foreach (PublishConfig config in targets)
            {
                ...
                foreach (string arch in config.Archs)
                {
                    ...
                    string projectDataDirName = $"data_{NebulaSharpDirs.CSharpProjectName}_{platform}_{arch}";
                    if (platform == OS.Platforms.MacOS)
                    {
                    ...
                    {
                        publishOutputDir = Path.Combine(Path.GetTempPath(), "nebula-publish-dotnet",
                            $"{System.Environment.ProcessId}-{buildConfig}-{runtimeIdentifier}");
                        _tempFolders.Add(publishOutputDir);
                    }
                    else
                    {
                        publishOutputDir = Path.Combine(NebulaSharpDirs.ProjectBaseOutputPath, "nebula-publish-dotnet",
                    ...
                    string assemblyPath = Path.Combine(publishOutputDir, $"{NebulaSharpDirs.ProjectAssemblyName}.dll");
                    string nativeAotPath = Path.Combine(publishOutputDir,
                        $"{NebulaSharpDirs.ProjectAssemblyName}.{soExt}");
                    ...
                    RecursePublishContents(publishOutputDir,
                        ...
                        filterFile: file =>
                        {
                            if (platform == OS.Platforms.iOS)
                            {
                                ...
                                return Path.GetFileName(file) != $"{NebulaSharpDirs.ProjectAssemblyName}.dylib";
                            }
                        ...
                        addEntry: (path, isFile) =>
                        {
                            ...
                            if (isFile)
                            {
                                if (embedBuildResults)
                                {
                                    ...
                                    AddFile($"res://.nebula/mono/publish/{arch}/{filePath}", fileData, false);
                                }
                                else
                                {
                    ...
                    if (embedBuildResults)
                    {
                        ...
                        AddFile($"res://.nebula/mono/publish/{arch}/.dotnet-publish-manifest", fileData, false);
                    }
                }
            }
            ...
            if (platform == OS.Platforms.iOS)
            {
                if (outputPaths.Count > 2)
                {
                    ...
                    lipoArgs.Add("-create");
                    lipoArgs.AddRange(outputPaths.Skip(1).Select(x => Path.Combine(x, $"{NebulaSharpDirs.ProjectAssemblyName}.dylib")));
                    lipoArgs.Add("-output");
                    lipoArgs.Add(Path.Combine(outputPaths[1], $"{NebulaSharpDirs.ProjectAssemblyName}.dylib"));
                ...
                var xcFrameworkPath = Path.Combine(NebulaSharpDirs.ProjectBaseOutputPath, publishConfig.BuildConfig,
                    $"{NebulaSharpDirs.ProjectAssemblyName}_aot.xcframework");
                if (!BuildManager.GenerateXCFrameworkBlocking(outputPaths,
                        Path.Combine(NebulaSharpDirs.ProjectBaseOutputPath, publishConfig.BuildConfig, xcFrameworkPath)))
                {
        ...
        public override void _ExportEnd()
        {
            ...
            string aotTempDir = Path.Combine(Path.GetTempPath(), $"nebula-aot-{System.Environment.ProcessId}");
            ...
            if (!string.IsNullOrEmpty(_maybeLastExportError))
            {
                ...
                NebulaSharpEditor.Instance.ShowErrorDialog(lastExportError, "Failed to export C# project");
            }
        }
