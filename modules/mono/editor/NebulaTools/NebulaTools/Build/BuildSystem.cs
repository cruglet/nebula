...
namespace NebulaTools.Build
{
    public static class BuildSystem
    {
        ...
        {
            var verbosityLevel =
                editorSettings.GetSetting(NebulaSharpEditor.Settings.VerbosityLevel).As<VerbosityLevelId>();
            arguments.Add("-v");
            ...
            if ((bool)editorSettings.GetSetting(NebulaSharpEditor.Settings.NoConsoleLogging))
                arguments.Add("-noconlog");
        }

        private static void AddLoggerArgument(BuildInfo buildInfo, Collection<string> arguments)
        {
            string buildLoggerPath = Path.Combine(Internals.NebulaSharpDirs.DataEditorToolsDir,
                "NebulaTools.BuildLogger.dll");

            arguments.Add(
                $"-l:{typeof(NebulaBuildLogger).FullName},{buildLoggerPath};{buildInfo.LogsDirPath}");
        }

        private static void AddBinaryLogArgument(BuildInfo buildInfo, Collection<string> arguments,
            EditorSettings editorSettings)
        {
            if (!(bool)editorSettings.GetSetting(NebulaSharpEditor.Settings.CreateBinaryLog))
                return;
        ...
        {
            var baseDylib = $"{NebulaSharpDirs.ProjectAssemblyName}.dylib";
            var baseSym = $"{NebulaSharpDirs.ProjectAssemblyName}.framework.dSYM";
