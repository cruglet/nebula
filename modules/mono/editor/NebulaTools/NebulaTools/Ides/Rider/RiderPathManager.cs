...
namespace NebulaTools.Ides.Rider
{
    public static class RiderPathManager
    {
        ...
        {
            var editorSettings = EditorInterface.Singleton.GetEditorSettings();
            var editor = editorSettings.GetSetting(NebulaSharpEditor.Settings.ExternalEditor).As<ExternalEditorId>();
            if (editor == ExternalEditorId.Rider)
            {
                if (!editorSettings.HasSetting(EditorPathSettingName))
                {
                    Globals.EditorDef(EditorPathSettingName, "");
                    editorSettings.AddPropertyInfo(new Nebula.Collections.Dictionary
                    {
