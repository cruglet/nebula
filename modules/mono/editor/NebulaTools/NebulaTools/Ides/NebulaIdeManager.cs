...
{
    public sealed partial class NebulaIdeManager : Node, ISerializationListener
    {
        ...
        private MessagingServer GetRunningOrNewServer()
        {
            ...
            _messagingServer = new MessagingServer(OS.GetExecutablePath(),
                ProjectSettings.GlobalizePath(NebulaSharpDirs.ResMetadataDir), new NebulaLogger());
        ...
        public async Task<EditorPick?> LaunchIdeAsync(int millisecondsTimeout = 10000)
        {
            ...
            var editorId = editorSettings.GetSetting(NebulaSharpEditor.Settings.ExternalEditor).As<ExternalEditorId>();
        ...
        private void LaunchIde(ExternalEditorId editorId, string editorIdentity)
        {
            switch (editorId)
            {
                ...
                case ExternalEditorId.MonoDevelop:
                {
                    ...
                    try
                    {
                        var instance = GetMonoDevelopInstance(NebulaSharpDirs.ProjectSlnPath);
        ...
        public readonly struct EditorPick
        {
            ...
            public bool IsAnyConnected() =>
                NebulaSharpEditor.Instance.NebulaIdeManager.GetRunningOrNewServer().IsAnyConnected(_identity);
                ...
                where TResponse : Response, new()
            {
                ...
                NebulaSharpEditor.Instance.NebulaIdeManager
        ...
        private class NebulaLogger : ILogger
        {
