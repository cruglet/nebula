...
using NebulaTools.IdeMessaging;
...
namespace NebulaTools.Ides
{
    public sealed class MessagingServer : IDisposable
    {
        ...
        public MessagingServer(string editorExecutablePath, string projectMetadataDir, ILogger logger)
        {
            ...
            _metaFilePath = Path.Combine(projectMetadataDir, NebulaIdeMetadata.DefaultFileName);

            // Make sure the directory exists
            Directory.CreateDirectory(projectMetadataDir);

            // The Nebula editor's file system thread can keep the file open for writing, so we are forced to allow write sharing...
            ...
            where TResponse : Response, new()
        {
            using (await _peersSem.UseAsync())
            {
                ...
                {
                    _logger.LogError("Cannot write request. No client connected to the Nebula Ide Server.");
                    return;
                }
        ...
        private class ServerMessageHandler : IMessageHandler
        {
            private static void DispatchToMainThread(Action action)
            {
                ...
                Nebula.Dispatcher.SynchronizationContext.Post(d, null);
            }
            ...
            private static Task<Response> HandleDebugPlay(DebugPlayRequest request)
            {
                DispatchToMainThread(() =>
                {
                    ...
                    NebulaSharpEditor.Instance.SkipBuildBeforePlaying = !(request.BuildBeforePlaying ?? true);
                    ...
                    Environment.SetEnvironmentVariable("NEBULA_MONO_DEBUGGER_AGENT",
                    ...
                    Environment.SetEnvironmentVariable("NEBULA_MONO_DEBUGGER_AGENT", "");
                    NebulaSharpEditor.Instance.SkipBuildBeforePlaying = false;
                });
            ...
            private static async Task<Response> HandleCodeCompletionRequest(CodeCompletionRequest request)
            {
                ...
                await Nebula.Engine.GetMainLoop().ToSignal(Nebula.Engine.GetMainLoop(), "process_frame");
