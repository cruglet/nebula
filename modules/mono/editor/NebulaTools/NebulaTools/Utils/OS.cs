...
namespace NebulaTools.Utils
{
    ...
    public static class OS
    {
        ...
        public static readonly Dictionary<string, string> DotNetOSPlatformMap = new Dictionary<string, string>
        {
            ...
            // Nebula has a single export profile for both, named LinuxBSD.
        ...
        {
            Internal.nebula_icall_Utils_OS_GetPlatformName(out nebula_string dest);
            using (dest)
            {
        ...
        private static string? PathWhichUnix(string name)
        {
            ...
            return searchDirs.Select(dir => Path.Combine(dir, name))
                .FirstOrDefault(path =>
                {
                    using nebula_string pathIn = Marshaling.ConvertStringToNative(path);
                    return File.Exists(path) && Internal.nebula_icall_Utils_OS_UnixFileHasExecutableAccess(pathIn);
                });
        }
