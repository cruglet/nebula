...
namespace NebulaTools.IdeMessaging.CLI
{
    internal static class Program
    {
        ...
        private static async Task<int> StartAsync(string[] args, Stream inputStream, Stream outputStream)
        {
            ...
            try
            {
                ...
                string nebulaProjectDir = args[0];

                if (!Directory.Exists(nebulaProjectDir))
                {
                    Logger.LogError($"The specified Nebula project directory does not exist: {nebulaProjectDir}");
                    return 1;
                }

                var forwarder = new ForwarderMessageHandler(outputWriter);

                using (var fwdClient = new Client("VisualStudioCode", nebulaProjectDir, forwarder, Logger))
                {
                    fwdClient.Start();
