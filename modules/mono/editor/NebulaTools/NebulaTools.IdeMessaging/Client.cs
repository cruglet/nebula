...
using NebulaTools.IdeMessaging.Utils;
...
namespace NebulaTools.IdeMessaging
{
    ...
    public sealed class Client : IDisposable
    {
        ...
        private NebulaIdeMetadata nebulaIdeMetadata;
        private readonly FileSystemWatcher fsWatcher;

        public string NebulaEditorExecutablePath => nebulaIdeMetadata.EditorExecutablePath;
        ...
        public Client(string identity, string nebulaProjectDir, IMessageHandler messageHandler, ILogger logger)
        {
            this.identity = identity;
            this.messageHandler = messageHandler;
            this.logger = logger;

            string projectMetadataDir = Path.Combine(nebulaProjectDir, ".nebula", "mono", "metadata");
            // FileSystemWatcher requires an existing directory
            if (!Directory.Exists(projectMetadataDir))
            {
                // Check if the non hidden version exists
                string nonHiddenProjectMetadataDir = Path.Combine(nebulaProjectDir, "nebula", "mono", "metadata");
            ...
            MetaFilePath = Path.Combine(projectMetadataDir, NebulaIdeMetadata.DefaultFileName);

            fsWatcher = new FileSystemWatcher(projectMetadataDir, NebulaIdeMetadata.DefaultFileName);
        }
        ...
        private async void OnMetaFileChanged(object sender, FileSystemEventArgs e)
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (metadata != null && metadata != nebulaIdeMetadata)
                {
                    nebulaIdeMetadata = metadata.Value;
        ...
        private NebulaIdeMetadata? ReadMetadataFile()
        {
            ...
            using (var reader = new StreamReader(fileStream))
            {
                ...
                return new NebulaIdeMetadata(port, editorExecutablePath);
            }
        }
        ...
        private async Task ConnectToServer()
        {
            ...
            try
            {
                logger.LogInfo("Connecting to Nebula Ide Server");

                await tcpClient.ConnectAsync(IPAddress.Loopback, nebulaIdeMetadata.Port);

                logger.LogInfo("Connection open with Nebula Ide Server");
            ...
            catch (SocketException e)
            {
                if (e.SocketErrorCode == SocketError.ConnectionRefused)
                    logger.LogError("The connection to the Nebula Ide Server was refused");
                else
                    throw;
            }
        }
        ...
        public async void Start()
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (!File.Exists(MetaFilePath))
                {
                    logger.LogInfo("There is no Nebula Ide Server running");
                    return;
                }
                ...
                {
                    logger.LogError("Failed to read Nebula Ide metadata file");
                }
            }
        }
            ...
            where TResponse : Response, new()
        {
            if (!IsConnected)
            {
                logger.LogError("Cannot write request. Not connected to the Nebula Ide Server.");
                return null;
            }
