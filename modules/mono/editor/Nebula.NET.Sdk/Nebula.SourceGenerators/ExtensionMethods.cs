...
namespace Nebula.SourceGenerators
{
    internal static class ExtensionMethods
    {
        ...
        public static bool AreNebulaSourceGeneratorsDisabled(this GeneratorExecutionContext context)
            => context.TryGetGlobalAnalyzerProperty("NebulaSourceGenerators", out string? toggle) &&
               toggle != null &&
               toggle.Equals("disabled", StringComparison.OrdinalIgnoreCase);

        public static bool IsNebulaToolsProject(this GeneratorExecutionContext context)
            => context.TryGetGlobalAnalyzerProperty("IsNebulaToolsProject", out string? toggle) &&
               toggle != null &&
               toggle.Equals("true", StringComparison.OrdinalIgnoreCase);

        public static bool IsNebulaSourceGeneratorDisabled(this GeneratorExecutionContext context, string generatorName) =>
            AreNebulaSourceGeneratorsDisabled(context) ||
            (context.TryGetGlobalAnalyzerProperty("NebulaDisabledSourceGenerators", out string? disabledGenerators) &&
        ...
        public static INamedTypeSymbol? GetNebulaScriptNativeClass(this INamedTypeSymbol classTypeSymbol)
        {
            var symbol = classTypeSymbol;

            while (symbol != null)
            {
                if (symbol.ContainingAssembly?.Name == "NebulaSharp")
                    return symbol;
        ...
        public static string? GetNebulaScriptNativeClassName(this INamedTypeSymbol classTypeSymbol)
        {
            var nativeType = classTypeSymbol.GetNebulaScriptNativeClass();

            if (nativeType == null)
                return null;

            var nebulaClassNameAttr = nativeType.GetAttributes()
                .FirstOrDefault(a => a.AttributeClass?.IsNebulaClassNameAttribute() ?? false);

            string? nebulaClassName = null;

            if (nebulaClassNameAttr is { ConstructorArguments: { Length: > 0 } })
                nebulaClassName = nebulaClassNameAttr.ConstructorArguments[0].Value?.ToString();

            return nebulaClassName ?? nativeType.Name;
        }

        private static bool TryGetNebulaScriptClass(
        ...
        )
        {
            ...
            if (classTypeSymbol?.BaseType == null
                || !classTypeSymbol.BaseType.InheritsFrom("NebulaSharp", NebulaClasses.NebulaObject))
            {
                symbol = null;
                return false;
            }
        ...
        public static IEnumerable<(ClassDeclarationSyntax cds, INamedTypeSymbol symbol)> SelectNebulaScriptClasses(
        ...
        )
        {
            ...
            {
                if (cds.TryGetNebulaScriptClass(compilation, out var symbol))
        ...
        public static bool IsNebulaExportAttribute(this INamedTypeSymbol symbol)
            => symbol.FullQualifiedNameOmitGlobal() == NebulaClasses.ExportAttr;

        public static bool IsNebulaSignalAttribute(this INamedTypeSymbol symbol)
            => symbol.FullQualifiedNameOmitGlobal() == NebulaClasses.SignalAttr;

        public static bool IsNebulaMustBeVariantAttribute(this INamedTypeSymbol symbol)
            => symbol.FullQualifiedNameOmitGlobal() == NebulaClasses.MustBeVariantAttr;

        public static bool IsNebulaClassNameAttribute(this INamedTypeSymbol symbol)
            => symbol.FullQualifiedNameOmitGlobal() == NebulaClasses.NebulaClassNameAttr;

        public static bool IsNebulaGlobalClassAttribute(this INamedTypeSymbol symbol)
            => symbol.FullQualifiedNameOmitGlobal() == NebulaClasses.GlobalClassAttr;

        public static bool IsSystemFlagsAttribute(this INamedTypeSymbol symbol)
            => symbol.FullQualifiedNameOmitGlobal() == NebulaClasses.SystemFlagsAttr;

        public static NebulaMethodData? HasNebulaCompatibleSignature(
        ...
        )
        {
            ...
            return new NebulaMethodData(method, paramTypes,
                parameters.Select(p => p.Type).ToImmutableArray(),
                retType != null ? (retType.Value, retSymbol) : null);
        }

        public static IEnumerable<NebulaMethodData> WhereHasNebulaCompatibleSignature(
        ...
        )
        {
            ...
            {
                var methodData = HasNebulaCompatibleSignature(method, typeCache);
        ...
        public static IEnumerable<NebulaPropertyData> WhereIsNebulaCompatibleType(
        ...
        )
        {
            foreach (var property in properties)
            {
                ...
                yield return new NebulaPropertyData(property, marshalType.Value);
            }
        }

        public static IEnumerable<NebulaFieldData> WhereIsNebulaCompatibleType(
        ...
        )
        {
            ...
            {
                // TODO: We should still restore read-only fields after reloading assembly. Two possible ways: reflection or turn RestoreNebulaObjectData into a constructor overload.
                var marshalType = MarshalUtils.ConvertManagedTypeToMarshalType(field.Type, typeCache);

                if (marshalType == null)
                    continue;

                yield return new NebulaFieldData(field, marshalType.Value);
            }
        }
