...
namespace Nebula.SourceGenerators
{
    internal static class MarshalUtils
    {
        ...
        {
            public INamedTypeSymbol NebulaObjectType { get; }
            ...
            public TypeCache(Compilation compilation)
            {
                ...
                NebulaObjectType = GetTypeByMetadataNameOrThrow(NebulaClasses.NebulaObject);
            }
        }
        ...
        public static VariantType? ConvertMarshalTypeToVariantType(MarshalType marshalType)
            => marshalType switch
            {
                ...
                MarshalType.NebulaObjectOrDerivedArray => VariantType.Array,
                ...
                MarshalType.NebulaObjectOrDerived => VariantType.Object,
                MarshalType.StringName => VariantType.StringName,
                MarshalType.NodePath => VariantType.NodePath,
                MarshalType.Rid => VariantType.Rid,
                MarshalType.NebulaDictionary => VariantType.Dictionary,
                MarshalType.NebulaArray => VariantType.Array,
                MarshalType.NebulaGenericDictionary => VariantType.Dictionary,
                MarshalType.NebulaGenericArray => VariantType.Array,
                _ => null
            };
        ...
        public static MarshalType? ConvertManagedTypeToMarshalType(ITypeSymbol type, TypeCache typeCache)
        {
            ...
            switch (specialType)
            {
                ...
                default:
                {
                    ...
                    {
                        if (type.ContainingAssembly?.Name == "NebulaSharp" &&
                            type.ContainingNamespace?.Name == "Nebula")
                        {
                            return type switch
                            {
                    ...
                    else if (typeKind == TypeKind.Array)
                    {
                        ...
                        if (elementType.SimpleDerivesFrom(typeCache.NebulaObjectType))
                            return MarshalType.NebulaObjectOrDerivedArray;

                        if (elementType.ContainingAssembly?.Name == "NebulaSharp" &&
                            elementType.ContainingNamespace?.Name == "Nebula")
                        {
                            switch (elementType)
                            {
                                case { Name: "Vector2" }:
                    ...
                    {
                        if (type.SimpleDerivesFrom(typeCache.NebulaObjectType))
                            return MarshalType.NebulaObjectOrDerived;
                        ...
                        if (type.ContainingAssembly?.Name == "NebulaSharp")
                        {
                            switch (type.ContainingNamespace?.Name)
                            {
                                ...
                                case "Collections"
                                    when type.ContainingNamespace?.FullQualifiedNameOmitGlobal() == "Nebula.Collections":
                                    return type switch
                                    {
                                        { Name: "Dictionary" } =>
                                            type is INamedTypeSymbol { IsGenericType: false } ?
                                                MarshalType.NebulaDictionary :
                                                MarshalType.NebulaGenericDictionary,
                                        { Name: "Array" } =>
                                            type is INamedTypeSymbol { IsGenericType: false } ?
                                                MarshalType.NebulaArray :
                                                MarshalType.NebulaGenericArray,
                                        _ => null
                                    };
                            }
                        }
        ...
        private const string VariantUtils = "global::Nebula.NativeInterop.VariantUtils";
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            ...
            {
                // We need a special case for NebulaObjectOrDerived[], because it's not supported by VariantUtils.ConvertTo<T>
                MarshalType.NebulaObjectOrDerivedArray =>
                    source.Append(VariantUtils, ".ConvertToSystemArrayOfNebulaObject<",
                        ((IArrayTypeSymbol)typeSymbol).ElementType.FullQualifiedNameIncludeGlobal(), ">(",
                        inputExpr, ")"),
                // We need a special case for generic Nebula collections and NebulaObjectOrDerived[], because VariantUtils.ConvertTo<T> is slower
                MarshalType.NebulaGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for NebulaObjectOrDerived[], because it's not supported by VariantUtils.CreateFrom<T>
                MarshalType.NebulaObjectOrDerivedArray =>
                    source.Append(VariantUtils, ".CreateFromSystemArrayOfNebulaObject(", inputExpr, ")"),
                // We need a special case for generic Nebula collections and NebulaObjectOrDerived[], because VariantUtils.CreateFrom<T> is slower
                MarshalType.NebulaGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for NebulaObjectOrDerived[], because it's not supported by Variant.As<T>
                MarshalType.NebulaObjectOrDerivedArray =>
                    source.Append(inputExpr, ".AsNebulaObjectArray<",
                        ((IArrayTypeSymbol)typeSymbol).ElementType.FullQualifiedNameIncludeGlobal(), ">()"),
                // We need a special case for generic Nebula collections and NebulaObjectOrDerived[], because Variant.As<T> is slower
                MarshalType.NebulaGenericDictionary =>
                    source.Append(inputExpr, ".AsNebulaDictionary<",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[0].FullQualifiedNameIncludeGlobal(), ", ",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[1].FullQualifiedNameIncludeGlobal(), ">()"),
                MarshalType.NebulaGenericArray =>
                    source.Append(inputExpr, ".AsNebulaArray<",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[0].FullQualifiedNameIncludeGlobal(), ">()"),
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for NebulaObjectOrDerived[], because it's not supported by Variant.From<T>
                MarshalType.NebulaObjectOrDerivedArray =>
                    source.Append("global::Nebula.Variant.CreateFrom(", inputExpr, ")"),
                // We need a special case for generic Nebula collections, because Variant.From<T> is slower
                MarshalType.NebulaGenericDictionary or MarshalType.NebulaGenericArray =>
                    source.Append("global::Nebula.Variant.CreateFrom(", inputExpr, ")"),
                _ => source.Append("global::Nebula.Variant.From<",
                    typeSymbol.FullQualifiedNameIncludeGlobal(), ">(", inputExpr, ")")
            };
        }
    }
