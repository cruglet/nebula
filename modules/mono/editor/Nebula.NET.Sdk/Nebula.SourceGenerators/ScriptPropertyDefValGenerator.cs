...
namespace Nebula.SourceGenerators
{
    ...
    public class ScriptPropertyDefValGenerator : ISourceGenerator
    {
        ...
        {
            if (context.IsNebulaSourceGeneratorDisabled("ScriptPropertyDefVal"))
                return;
        ...
        )
        {
            ...
            bool isNode = symbol.InheritsFrom("NebulaSharp", NebulaClasses.Node);
            ...
            var exportedProperties = members
                ...
                .Where(s => s.GetAttributes()
                    .Any(a => a.AttributeClass?.IsNebulaExportAttribute() ?? false))
                .ToArray();
            ...
            foreach (var property in exportedProperties)
            {
                ...
                // Two possible ways: reflection or turn RestoreNebulaObjectData into a constructor overload.
                // Ignore properties without a getter, without a setter or with an init-only setter.
                // Nebula properties must be both readable and writable.
            ...
            foreach (var field in exportedFields)
            {
                ...
                // Ignore properties without a getter or without a setter. Nebula properties must be both readable and writable.
                if (field.IsReadOnly)
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        Common.ExportedMemberIsReadOnlyRule,
            ...
            // Generate GetNebulaExportedProperties
            ...
            if (exportedMembers.Count > 0)
            {
                ...
                const string DictionaryType =
                    "global::System.Collections.Generic.Dictionary<global::Nebula.StringName, global::Nebula.Variant>";

                source.Append("#if TOOLS\n");

                source.Append("    /// <summary>\n")
                    .Append("    /// Get the default values for all properties declared in this class.\n")
                    .Append("    /// This method is used by Nebula to determine the value that will be\n")
                ...
                source.Append(" GetNebulaPropertyDefaultValues()\n    {\n");
        ...
        {
            if (marshalType == MarshalType.NebulaObjectOrDerived)
            {
                return memberType.InheritsFrom("NebulaSharp", NebulaClasses.Node);
            }
            if (marshalType == MarshalType.NebulaObjectOrDerivedArray)
            {
                var elementType = ((IArrayTypeSymbol)memberType).ElementType;
                return elementType.InheritsFrom("NebulaSharp", NebulaClasses.Node);
            }
            if (memberType is INamedTypeSymbol { IsGenericType: true } genericType)
            {
                return genericType.TypeArguments
                    .Any(static typeArgument
                        => typeArgument.InheritsFrom("NebulaSharp", NebulaClasses.Node));
            }
