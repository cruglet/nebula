...
namespace Nebula.SourceGenerators
{
    ...
    public class ScriptPropertiesGenerator : ISourceGenerator
    {
        ...
        {
            if (context.IsNebulaSourceGeneratorDisabled("ScriptProperties"))
                return;
        ...
        )
        {
            ...
            var nebulaClassProperties = propertySymbols.WhereIsNebulaCompatibleType(typeCache).ToArray();
            var nebulaClassFields = fieldSymbols.WhereIsNebulaCompatibleType(typeCache).ToArray();
            ...
            foreach (var property in nebulaClassProperties)
            {
                ...
                source.Append("        public new static readonly global::Nebula.StringName @");
            ...
            source.Append("    }\n"); // class NebulaInternal

            if (nebulaClassProperties.Length > 0 || nebulaClassFields.Length > 0)
            {

                // Generate SetNebulaClassPropertyValue

                bool allPropertiesAreReadOnly = nebulaClassFields.All(fi => fi.FieldSymbol.IsReadOnly) &&
                                                nebulaClassProperties.All(pi => pi.PropertySymbol.IsReadOnly || pi.PropertySymbol.SetMethod!.IsInitOnly);

                if (!allPropertiesAreReadOnly)
                {
                    source.Append("    /// <inheritdoc/>\n");
                    source.Append("    [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]\n");
                    source.Append("    protected override bool SetNebulaClassPropertyValue(in nebula_string_name name, ");
                    source.Append("in nebula_variant value)\n    {\n");
                    ...
                    source.Append("        return base.SetNebulaClassPropertyValue(name, value);\n");

                    source.Append("    }\n");
                }

                // Generate GetNebulaClassPropertyValue
                bool allPropertiesAreWriteOnly = nebulaClassFields.Length == 0 && nebulaClassProperties.All(pi => pi.PropertySymbol.IsWriteOnly);

                if (!allPropertiesAreWriteOnly)
                {
                    source.Append("    /// <inheritdoc/>\n");
                    source.Append("    [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]\n");
                    source.Append("    protected override bool GetNebulaClassPropertyValue(in nebula_string_name name, ");
                    source.Append("out nebula_variant value)\n    {\n");
                    ...
                    source.Append("        return base.GetNebulaClassPropertyValue(name, out value);\n");

                    source.Append("    }\n");
                }
                // Generate GetNebulaPropertyList

                const string DictionaryType = "global::System.Collections.Generic.List<global::Nebula.Bridge.PropertyInfo>";

                source.Append("    /// <summary>\n")
                    .Append("    /// Get the property information for all the properties declared in this class.\n")
                    .Append("    /// This method is used by Nebula to register the available properties in the editor.\n")
                ...
                var nebulaClassPropertiesAndFields = Enumerable.Empty<NebulaPropertyOrFieldData>()
                    .Concat(nebulaClassProperties.Select(propertyData => new NebulaPropertyOrFieldData(propertyData)))
                    .Concat(nebulaClassFields.Select(fieldData => new NebulaPropertyOrFieldData(fieldData)))
                    .OrderBy(data => data.Symbol.Locations[0].Path())
                    .ThenBy(data => data.Symbol.Locations[0].StartLine());

                foreach (var member in nebulaClassPropertiesAndFields)
                {
        ...
        {
            source.Append("        properties.Add(new(type: (global::Nebula.Variant.Type)")
                .Append((int)VariantType.Nil)
                .Append(", name: \"")
                .Append(propertyInfo.Name)
                .Append("\", hint: (global::Nebula.PropertyHint)")
                .Append((int)PropertyHint.None)
                .Append(", hintString: \"")
                .Append(propertyInfo.HintString)
                .Append("\", usage: (global::Nebula.PropertyUsageFlags)")
        ...
        private static IEnumerable<PropertyInfo> DetermineGroupingPropertyInfo(ISymbol memberSymbol)
        {
            foreach (var attr in memberSymbol.GetAttributes())
            {
                ...
                {
                    NebulaClasses.ExportCategoryAttr => PropertyUsageFlags.Category,
                    NebulaClasses.ExportGroupAttr => PropertyUsageFlags.Group,
                    NebulaClasses.ExportSubgroupAttr => PropertyUsageFlags.Subgroup,
                    _ => null
                };
        ...
        )
        {
            var exportAttr = memberSymbol.GetAttributes()
                .FirstOrDefault(a => a.AttributeClass?.IsNebulaExportAttribute() ?? false);
        ...
        )
        {
            ...
            if (variantType == VariantType.Object && type is INamedTypeSymbol memberNamedType)
            {
                ...
                if (memberNamedType.InheritsFrom("NebulaSharp", "Nebula.Resource"))
                {
                ...
                if (memberNamedType.InheritsFrom("NebulaSharp", "Nebula.Node"))
                {
            ...
            static string GetTypeName(INamedTypeSymbol memberSymbol)
            {
                if (memberSymbol.GetAttributes()
                    .Any(a => a.AttributeClass?.IsNebulaGlobalClassAttribute() ?? false))
                {
                    return memberSymbol.Name;
                }

                return memberSymbol.GetNebulaScriptNativeClassName()!;
            }
