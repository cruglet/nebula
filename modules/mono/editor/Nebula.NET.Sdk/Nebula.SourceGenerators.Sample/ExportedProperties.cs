...
namespace Nebula.SourceGenerators.Sample
{
    ...
    public partial class ExportedProperties : NebulaObject
    {
        ...
        [Export]
        private Nebula.Collections.Dictionary PropertyNebulaDictionary { get; set; } =
            new() { { "foo", 10 }, { Vector2.Up, Colors.Chocolate } };

        [Export]
        private Nebula.Collections.Array PropertyNebulaArray { get; set; } =
            new() { "foo", 10, Vector2.Up, Colors.Chocolate };

        [Export]
        private Nebula.Collections.Dictionary<string, bool> PropertyNebulaGenericDictionary { get; set; } =
            new() { { "foo", true }, { "bar", false } };

        [Export]
        private Nebula.Collections.Array<int> PropertyNebulaGenericArray { get; set; } =
            new() { 0, 1, 2, 3, 4, 5, 6 };
    }
}
