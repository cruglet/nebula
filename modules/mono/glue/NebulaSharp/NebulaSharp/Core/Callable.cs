...
namespace Nebula
{
    ...
    {
        private readonly NebulaObject _target;
        private readonly StringName _method;
        private readonly Delegate _delegate;
        private readonly unsafe delegate* managed<object, NativeVariantPtrArgs, out nebula_variant, void> _trampoline;

        /// <summary>
        /// Object that contains the method.
        /// </summary>
        public NebulaObject Target => _target;
        ...
        /// </summary>
        public unsafe delegate* managed<object, NativeVariantPtrArgs, out nebula_variant, void> Trampoline
            => _trampoline;
        ...
        public unsafe Callable(NebulaObject target, StringName method)
        {
            _target = target;
            _method = method;
            _delegate = null;
        ...
        {
            _target = @delegate?.Target as NebulaObject;
            _method = null;
            _delegate = @delegate;
        ...
        {
            using nebula_callable callable = Marshaling.ConvertCallableToNative(this);

            int argc = args.Length;

            Span<nebula_variant.movable> argsStoreSpan = argc <= VarArgsSpanThreshold ?
                stackalloc nebula_variant.movable[VarArgsSpanThreshold] :
                new nebula_variant.movable[argc];

            Span<IntPtr> argsSpan = argc <= VarArgsSpanThreshold ?
                stackalloc IntPtr[VarArgsSpanThreshold] :
                new IntPtr[argc];

            fixed (nebula_variant* varargs = &MemoryMarshal.GetReference(argsStoreSpan).DangerousSelfRef)
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                for (int i = 0; i < argc; i++)
                {
                    varargs[i] = (nebula_variant)args[i].NativeVar;
                    argsPtr[i] = new IntPtr(&varargs[i]);
                }

                nebula_variant ret = NativeFuncs.nebulasharp_callable_call(callable,
                    (nebula_variant**)argsPtr, argc, out nebula_variant_call_error vcall_error);
                ExceptionUtils.DebugCheckCallError(callable, (nebula_variant**)argsPtr, argc, vcall_error);
        ...
        public unsafe void CallDeferred(params Variant[] args)
        {
            ...
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                ...
                NativeFuncs.nebulasharp_callable_call_deferred(callable, (nebula_variant**)argsPtr, argc);
            }
        }
        ...
        ///    <item>
        ///        <term>args</term>
        ///        <description>Array of <see cref="nebula_variant"/> arguments.</description>
        ///    </item>
        ///    <item>
        ///        <term>ret</term>
        ///        <description>Return value of type <see cref="nebula_variant"/>.</description>
        ///    </item>
        ///</list>
        /// <para>
        ...
        ///
        /// <code>
        ///     static void Trampoline(object delegateObj, NativeVariantPtrArgs args, out nebula_variant ret)
        ///     {
        ///         if (args.Count != 1)
