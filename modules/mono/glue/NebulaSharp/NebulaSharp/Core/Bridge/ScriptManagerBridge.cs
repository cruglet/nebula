...
namespace Nebula.Bridge
{
    ...
    public static partial class ScriptManagerBridge
    {
        ...
        internal static void FrameCallback()
        {
            try
            {
                Dispatcher.DefaultNebulaTaskScheduler?.Activate();
            }
            catch (Exception e)
            {
        ...
        [UnmanagedCallersOnly]
        internal static unsafe IntPtr CreateManagedForNebulaObjectBinding(nebula_string_name* nativeTypeName,
            IntPtr nebulaObject)
        {
            ...
            try
            {
                using var stringName = StringName.CreateTakingOwnershipOfDisposableValue(
                    NativeFuncs.nebulasharp_string_name_new_copy(CustomUnsafe.AsRef(nativeTypeName)));
                ...
                var obj = (NebulaObject)FormatterServices.GetUninitializedObject(nativeType);

                var ctor = nativeType.GetConstructor(
                    BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance,
                    null, Type.EmptyTypes, null);

                obj.NativePtr = nebulaObject;
        ...
        [UnmanagedCallersOnly]
        internal static unsafe nebula_bool CreateManagedForNebulaObjectScriptInstance(IntPtr scriptPtr,
            IntPtr nebulaObject,
            nebula_variant** args, int argCount)
        {
            // TODO: Optimize with source generators and delegate pointers.
            ...
            try
            {
                ...
                var obj = (NebulaObject)FormatterServices.GetUninitializedObject(scriptType);
        ...
        [UnmanagedCallersOnly]
        internal static unsafe void GetScriptNativeName(IntPtr scriptPtr, nebula_string_name* outRes)
        {
            try
            {
                // Performance is not critical here as this will be replaced with source generators.
                ...
                var native = NebulaObject.InternalGetClassNativeBase(scriptType);
                ...
                *outRes = NativeFuncs.nebulasharp_string_name_new_copy((nebula_string_name)nativeName.NativeValue);
            }
            catch (Exception e)
            {
                ExceptionUtils.LogException(e);
        ...
        [UnmanagedCallersOnly]
        internal static unsafe void GetGlobalClassName(nebula_string* scriptPath, nebula_string* outBaseType, nebula_string* outIconPath, nebula_string* outClassName)
        {
            ...
            if (outBaseType != null)
            {
                ...
                Type native = NebulaObject.InternalGetClassNativeBase(scriptType);
        ...
        [UnmanagedCallersOnly]
        internal static void SetNebulaObjectPtr(IntPtr gcHandlePtr, IntPtr newPtr)
        {
            try
            {
                var target = (NebulaObject?)GCHandle.FromIntPtr(gcHandlePtr).Target;
                if (target != null)
        ...
        private static Type? TypeGetProxyClass(string nativeTypeNameStr)
        {
            ...
            Type? wrapperType = typeof(NebulaObject).Assembly.GetType("Nebula." + nativeTypeNameStr);

            if (wrapperType == null)
            {
                wrapperType = GetTypeByNebulaClassAttr(typeof(NebulaObject).Assembly, nativeTypeNameStr);
            }

            if (wrapperType == null)
            {
                var editorAssembly = AppDomain.CurrentDomain.GetAssemblies()
                    .FirstOrDefault(a => a.GetName().Name == "NebulaSharpEditor");

                if (editorAssembly != null)
                {
                    wrapperType = editorAssembly.GetType("Nebula." + nativeTypeNameStr);

                    if (wrapperType == null)
                    {
                        wrapperType = GetTypeByNebulaClassAttr(editorAssembly, nativeTypeNameStr);
                    }
                }
            }

            static Type? GetTypeByNebulaClassAttr(Assembly assembly, string nativeTypeNameStr)
            {
                var types = assembly.GetTypes();
                foreach (var type in types)
                {
                    var attr = type.GetCustomAttribute<NebulaClassNameAttribute>();
            ...
            {
                // A static class means this is a Nebula singleton class. Try to get the Instance proxy type.
                wrapperType = TypeGetProxyClass($"{wrapperType.Name}Instance");
                if (wrapperType == null)
                {
                    // Otherwise, fallback to NebulaObject.
                    return typeof(NebulaObject);
                }
            }
        ...
        public static void LookupScriptsInAssembly(Assembly assembly)
        {
            ...
            if (assemblyHasScriptsAttr.RequiresLookup)
            {
                ...
                var typeOfNebulaObject = typeof(NebulaObject);
                ...
                foreach (var type in assembly.GetTypes())
                {
                    ...
                    if (!typeOfNebulaObject.IsAssignableFrom(type))
                        continue;
            ...
            if (NativeFuncs.nebulasharp_dotnet_module_is_initialized().ToBool() && Engine.IsEditorHint())
            {
                if (_pathTypeBiMap.Paths.Count > 0)
                {
                    string[] scriptPaths = _pathTypeBiMap.Paths.ToArray();
                    using nebula_packed_string_array scriptPathsNative = Marshaling.ConvertSystemArrayToNativePackedStringArray(scriptPaths);
                    NativeFuncs.nebulasharp_internal_editor_file_system_update_files(scriptPathsNative);
                }
            }
        }
        ...
        internal static unsafe void RaiseEventSignal(IntPtr ownerGCHandlePtr,
            nebula_string_name* eventSignalName, nebula_variant** args, int argCount, nebula_bool* outOwnerIsNull)
        {
            try
            {
                var owner = (NebulaObject?)GCHandle.FromIntPtr(ownerGCHandlePtr).Target;

                if (owner == null)
                {
                    *outOwnerIsNull = nebula_bool.True;
                    return;
                }

                *outOwnerIsNull = nebula_bool.False;

                owner.RaiseNebulaClassSignalCallbacks(CustomUnsafe.AsRef(eventSignalName),
        ...
        [UnmanagedCallersOnly]
        internal static nebula_bool ScriptIsOrInherits(IntPtr scriptPtr, IntPtr scriptPtrMaybeBase)
        {
            try
            {
                if (!_scriptTypeBiMap.TryGetScriptType(scriptPtr, out Type? scriptType))
                ...
                return (scriptType == maybeBaseType || maybeBaseType.IsAssignableFrom(scriptType)).ToNebulaBool();
            }
            catch (Exception e)
            {
                ExceptionUtils.LogException(e);
        ...
        [UnmanagedCallersOnly]
        internal static unsafe nebula_bool AddScriptBridge(IntPtr scriptPtr, nebula_string* scriptPath)
        {
            try
            {
                string scriptPathStr = Marshaling.ConvertStringToManaged(*scriptPath);
                return AddScriptBridgeCore(scriptPtr, scriptPathStr).ToNebulaBool();
            }
            catch (Exception e)
            {
                ExceptionUtils.LogException(e);
        ...
        [UnmanagedCallersOnly]
        internal static unsafe void GetOrCreateScriptBridgeForPath(nebula_string* scriptPath, nebula_ref* outScript)
        {
            string scriptPathStr = Marshaling.ConvertStringToManaged(*scriptPath);

            if (!_pathTypeBiMap.TryGetScriptType(scriptPathStr, out Type? scriptType))
            {
                NativeFuncs.nebulasharp_internal_new_csharp_script(outScript);
                return;
            }
        ...
        private static unsafe void GetOrCreateScriptBridgeForType(Type scriptType, nebula_ref* outScript)
        {
            lock (_scriptTypeBiMap.ReadWriteLock)
            {
                ...
                {
                    // Use existing
                    NativeFuncs.nebulasharp_ref_new_from_ref_counted_ptr(out *outScript, scriptPtr);
                    return;
                }
        ...
        internal static unsafe void GetOrLoadOrCreateScriptForType(Type scriptType, nebula_ref* outScript)
        {
            static bool GetPathOtherwiseGetOrCreateScript(Type scriptType, nebula_ref* outScript,
            ...
            {
                // Constructed generic types all have the same path which is not allowed by Nebula
            ...
            if (GetPathOtherwiseGetOrCreateScript(scriptType, outScript, out string? scriptPath))
            {
                ...
                using nebula_string scriptPathIn = Marshaling.ConvertStringToNative(scriptPath);
                if (!NativeFuncs.nebulasharp_internal_script_load(scriptPathIn, outScript).ToBool())
                {
        ...
        private static unsafe void CreateScriptBridgeForType(Type scriptType, nebula_ref* outScript)
        {
            ...
            NativeFuncs.nebulasharp_internal_reload_registered_script(scriptPtr);
        }
        ...
        [UnmanagedCallersOnly]
        internal static nebula_bool TryReloadRegisteredScriptWithClass(IntPtr scriptPtr)
        {
            try
            {
                lock (_scriptTypeBiMap.ReadWriteLock)
                {
                    ...
                    if (!typeof(NebulaObject).IsAssignableFrom(scriptType))
                    {
                        // The class no longer inherits NebulaObject, can't reload
                        return nebula_bool.False;
                    }
        ...
        private static unsafe void GetScriptTypeInfo(Type scriptType, nebula_csharp_type_info* outTypeInfo)
        {
            Type native = NebulaObject.InternalGetClassNativeBase(scriptType);
            ...
            nebula_string className = Marshaling.ConvertStringToNative(typeName);
            ...
            // Every script in the NebulaTools assembly is a tool script.
            if (!isTool && scriptType.Assembly.GetName().Name == "NebulaTools")
            {
                isTool = true;
            }
            ...
            nebula_string iconPath = Marshaling.ConvertStringToNative(iconAttr?.Path);

            outTypeInfo->ClassName = className;
            outTypeInfo->IconPath = iconPath;
            outTypeInfo->IsTool = isTool.ToNebulaBool();
            outTypeInfo->IsGlobalClass = isGlobalClass.ToNebulaBool();
            outTypeInfo->IsAbstract = scriptType.IsAbstract.ToNebulaBool();
            outTypeInfo->IsGenericTypeDefinition = scriptType.IsGenericTypeDefinition.ToNebulaBool();
            outTypeInfo->IsConstructedGenericType = scriptType.IsConstructedGenericType.ToNebulaBool();
        ...
        [UnmanagedCallersOnly]
        internal static unsafe void UpdateScriptClassInfo(IntPtr scriptPtr, nebula_csharp_type_info* outTypeInfo,
            nebula_array* outMethodsDest, nebula_dictionary* outRpcFunctionsDest, nebula_dictionary* outEventSignalsDest, nebula_ref* outBaseScript)
        {
            try
            {
                // Performance is not critical here as this will be replaced with source generators.
                ...
                *outMethodsDest = NativeFuncs.nebulasharp_array_new_copy(
                    (nebula_array)methods.NativeValue);
                ...
                *outRpcFunctionsDest = NativeFuncs.nebulasharp_dictionary_new_copy(
                    (nebula_dictionary)rpcFunctions.NativeValue);
                ...
                *outEventSignalsDest = NativeFuncs.nebulasharp_dictionary_new_copy(
                    (nebula_dictionary)signals.NativeValue);
            ...
            catch (Exception e)
            {
                ExceptionUtils.LogException(e);
                *outTypeInfo = default;
                *outMethodsDest = NativeFuncs.nebulasharp_array_new();
                *outRpcFunctionsDest = NativeFuncs.nebulasharp_dictionary_new();
                *outEventSignalsDest = NativeFuncs.nebulasharp_dictionary_new();
                *outBaseScript = default;
            }
        }
        ...
        {
            var getNebulaSignalListMethod = type.GetMethod(
                "GetNebulaSignalList",
                BindingFlags.DeclaredOnly | BindingFlags.Static |
                BindingFlags.NonPublic | BindingFlags.Public);

            if (getNebulaSignalListMethod == null)
                return null;

            return (List<MethodInfo>?)getNebulaSignalListMethod.Invoke(null, null);
        }

        private static List<MethodInfo>? GetMethodListForType(Type type)
        {
            var getNebulaMethodListMethod = type.GetMethod(
                "GetNebulaMethodList",
                BindingFlags.DeclaredOnly | BindingFlags.Static |
                BindingFlags.NonPublic | BindingFlags.Public);

            if (getNebulaMethodListMethod == null)
                return null;

            return (List<MethodInfo>?)getNebulaMethodListMethod.Invoke(null, null);
        }
...
#pragma warning disable IDE1006 // Naming rule violation
        ...
        private ref struct nebulasharp_property_info
        {
            // Careful with padding...
            public nebula_string_name Name; // Not owned
            public nebula_string HintString;
            public int Type;
            public int Hint;
            public int Usage;
            public nebula_bool Exported;
...
#pragma warning restore IDE1006
        ...
        internal static unsafe void GetPropertyInfoList(IntPtr scriptPtr,
            delegate* unmanaged<IntPtr, nebula_string*, void*, int, void> addPropInfoFunc)
        {
            try
            {
                Type scriptType = _scriptTypeBiMap.GetScriptType(scriptPtr);
            ...
            delegate* unmanaged<IntPtr, nebula_string*, void*, int, void> addPropInfoFunc)
        {
            try
            {
                var getNebulaPropertyListMethod = type.GetMethod(
                    "GetNebulaPropertyList",
                    BindingFlags.DeclaredOnly | BindingFlags.Static |
                    BindingFlags.NonPublic | BindingFlags.Public);

                if (getNebulaPropertyListMethod == null)
                    return;

                var properties = (List<PropertyInfo>?)
                    getNebulaPropertyListMethod.Invoke(null, null);
                ...
                // StackMaxSize = StackMaxLength * sizeof(nebulasharp_property_info)
                const int StackMaxLength = 32;
                bool useStack = length < StackMaxLength;

                nebulasharp_property_info* interopProperties;
                ...
                if (useStack)
                {
                    ...
                    var aux = stackalloc nebulasharp_property_info[StackMaxLength];
                    interopProperties = aux;
                }
                else
                {
                    interopProperties = ((nebulasharp_property_info*)NativeMemory.Alloc(
                        (nuint)length, (nuint)sizeof(nebulasharp_property_info)))!;
                }
                ...
                try
                {
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        nebulasharp_property_info interopProperty = new()
                        {
                            Type = (int)property.Type,
                            Name = (nebula_string_name)property.Name.NativeValue, // Not owned
                            Hint = (int)property.Hint,
                            HintString = Marshaling.ConvertStringToNative(property.HintString),
                            Usage = (int)property.Usage,
                            Exported = property.Exported.ToNebulaBool()
                        };

                        interopProperties[i] = interopProperty;
                    }

                    using nebula_string currentClassName = Marshaling.ConvertStringToNative(type.Name);
...
#pragma warning disable IDE1006 // Naming rule violation
        ...
        // ReSharper disable once NotAccessedField.Local
        [StructLayout(LayoutKind.Sequential)]
        private ref struct nebulasharp_property_def_val_pair
        {
            // Careful with padding...
            public nebula_string_name Name; // Not owned
            public nebula_variant Value; // Not owned
        }
#pragma warning restore IDE1006

        private delegate bool InvokeNebulaClassStaticMethodDelegate(in nebula_string_name method, NativeVariantPtrArgs args, out nebula_variant ret);

        [UnmanagedCallersOnly]
        internal static unsafe nebula_bool CallStatic(IntPtr scriptPtr, nebula_string_name* method,
            nebula_variant** args, int argCount, nebula_variant_call_error* refCallError, nebula_variant* ret)
        {
            ...
            try
            {
                ...
                Type? top = scriptType;
                Type native = NebulaObject.InternalGetClassNativeBase(top);

                while (top != null && top != native)
                {
                    var invokeNebulaClassStaticMethod = top.GetMethod(
                        "InvokeNebulaClassStaticMethod",
                        BindingFlags.DeclaredOnly | BindingFlags.Static |
                        BindingFlags.NonPublic | BindingFlags.Public);

                    if (invokeNebulaClassStaticMethod != null)
                    {
                        var invoked = invokeNebulaClassStaticMethod.CreateDelegate<InvokeNebulaClassStaticMethodDelegate>()(
                            CustomUnsafe.AsRef(method), new NativeVariantPtrArgs(args, argCount), out nebula_variant retValue);
                        if (invoked)
                        {
                            *ret = retValue;
                            return nebula_bool.True;
            ...
            delegate* unmanaged<IntPtr, void*, int, void> addDefValFunc)
        {
            try
            {
                var getNebulaPropertyDefaultValuesMethod = type.GetMethod(
                    "GetNebulaPropertyDefaultValues",
                    BindingFlags.DeclaredOnly | BindingFlags.Static |
                    BindingFlags.NonPublic | BindingFlags.Public);

                if (getNebulaPropertyDefaultValuesMethod == null)
                    return;

                var defaultValuesObj = getNebulaPropertyDefaultValuesMethod.Invoke(null, null);
                ...
                if (defaultValuesObj is Dictionary<StringName, object> defaultValuesLegacy)
                {
                    ...
                    // built with previous releases. Ideally, we should remove this before Nebula 4.0 stable.
                ...
                // There's no recursion here, so it's ok to go with a big enough number for most cases
                // StackMaxSize = StackMaxLength * sizeof(nebulasharp_property_def_val_pair)
                const int StackMaxLength = 32;
                bool useStack = length < StackMaxLength;

                nebulasharp_property_def_val_pair* interopDefaultValues;

                if (useStack)
                {
                    // Weird limitation, hence the need for aux:
                    // "In the case of pointer types, you can use a stackalloc expression only in a local variable declaration to initialize the variable."
                    var aux = stackalloc nebulasharp_property_def_val_pair[StackMaxLength];
                    interopDefaultValues = aux;
                }
                else
                {
                    interopDefaultValues = ((nebulasharp_property_def_val_pair*)NativeMemory.Alloc(
                        (nuint)length, (nuint)sizeof(nebulasharp_property_def_val_pair)))!;
                }
                ...
                try
                {
                    ...
                    {
                        nebulasharp_property_def_val_pair interopProperty = new()
                        {
                            Name = (nebula_string_name)defaultValuePair.Key.NativeValue, // Not owned
                            Value = (nebula_variant)defaultValuePair.Value.NativeVar // Not owned
                        };
        ...
        [UnmanagedCallersOnly]
        internal static unsafe nebula_bool SwapGCHandleForType(IntPtr oldGCHandlePtr, IntPtr* outNewGCHandlePtr,
            nebula_bool createWeak)
        {
            try
            {
                ...
                if (target == null)
                {
                    ...
                    *outNewGCHandlePtr = IntPtr.Zero;
                    return nebula_bool.False; // Called after the managed side was collected, so nothing to do here
                }
