...
public sealed class NebulaSerializationInfo : IDisposable
{
    ...
    private NebulaSerializationInfo(in nebula_dictionary properties, in nebula_dictionary signalEvents)
    {
    ...
    internal static NebulaSerializationInfo CreateCopyingBorrowed(
        in nebula_dictionary properties, in nebula_dictionary signalEvents)
    {
        return new(NativeFuncs.nebulasharp_dictionary_new_copy(properties),
            NativeFuncs.nebulasharp_dictionary_new_copy(signalEvents));
    }
        ...
        where T : Delegate
    {
        ...
        {
            if (DelegateUtils.TryDeserializeDelegate(serializedData.AsNebulaArray(), out var eventDelegate))
            {
