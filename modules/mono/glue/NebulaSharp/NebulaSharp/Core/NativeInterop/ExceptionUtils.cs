...
namespace Nebula.NativeInterop
{
    internal static class ExceptionUtils
    {
        ...
        private static void SendToScriptDebugger(Exception e)
        {
            ...
            using nebula_string nFile = Marshaling.ConvertStringToNative(file);
            using nebula_string nFunc = Marshaling.ConvertStringToNative(func);
            using nebula_string nErrorMsg = Marshaling.ConvertStringToNative(errorMsg);
            using nebula_string nExcMsg = Marshaling.ConvertStringToNative(excMsg.ToString());

            using DebuggingUtils.nebula_stack_info_vector stackInfoVector = default;
            ...
            unsafe
            {
                ...
                {
                    DebuggingUtils.nebula_stack_info* stackInfo = &stackInfoVector.Elements[i];
                ...
                NativeFuncs.nebulasharp_internal_script_debugger_send_error(nFunc, nFile, line,
                    nErrorMsg, nExcMsg, nebula_error_handler_type.ERR_HANDLER_ERROR, stackInfoVector);
            }
        }
        ...
        public static void LogException(Exception e)
        {
            try
            {
                if (NativeFuncs.nebulasharp_internal_script_debugger_is_active().ToBool())
                {
        ...
        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(nebula_string_name method, IntPtr instance, nebula_variant** args, int argCount, nebula_variant_call_error error)
        {
            if (error.Error != nebula_variant_call_error_error.NEBULA_CALL_ERROR_CALL_OK)
            {
                using nebula_variant instanceVariant = VariantUtils.CreateFromNebulaObjectPtr(instance);
        ...
        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(in nebula_callable callable, nebula_variant** args, int argCount, nebula_variant_call_error error)
        {
            if (error.Error != nebula_variant_call_error_error.NEBULA_CALL_ERROR_CALL_OK)
            {
                using nebula_variant callableVariant = VariantUtils.CreateFromCallableTakingOwnershipOfDisposableValue(callable);
        ...
        private unsafe static string GetCallErrorWhere(ref nebula_variant_call_error error, nebula_string_name method, nebula_variant* instance, nebula_variant** args, int argCount)
        {
            string? methodstr = null;
            string basestr = GetVariantTypeName(instance);

            if (method == NebulaObject.MethodName.Call || (basestr == "Nebula.TreeItem" && method == TreeItem.MethodName.CallRecursive))
            {
                if (argCount >= 1)
                {
                    methodstr = VariantUtils.ConvertToString(*args[0]);
                    if (error.Error == nebula_variant_call_error_error.NEBULA_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT)
                    {
                        error.Argument += 1;
                    }
                }
        ...
        private unsafe static string GetCallErrorMessage(nebula_variant_call_error error, string where, nebula_variant** args)
        {
            switch (error.Error)
            {
                case nebula_variant_call_error_error.NEBULA_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT:
                {
                ...
                case nebula_variant_call_error_error.NEBULA_CALL_ERROR_CALL_ERROR_TOO_MANY_ARGUMENTS:
                case nebula_variant_call_error_error.NEBULA_CALL_ERROR_CALL_ERROR_TOO_FEW_ARGUMENTS:
                    return $"Invalid call to {where}. Expected {error.Expected} arguments.";
                case nebula_variant_call_error_error.NEBULA_CALL_ERROR_CALL_ERROR_INVALID_METHOD:
                    return $"Invalid call. Nonexistent {where}.";
                case nebula_variant_call_error_error.NEBULA_CALL_ERROR_CALL_ERROR_INSTANCE_IS_NULL:
                    return $"Attempt to call {where} on a null instance.";
                case nebula_variant_call_error_error.NEBULA_CALL_ERROR_CALL_ERROR_METHOD_NOT_CONST:
        ...
        private unsafe static string GetVariantTypeName(nebula_variant* variant)
        {
            if (variant->Type == Variant.Type.Object)
            {
                NebulaObject obj = VariantUtils.ConvertToNebulaObject(*variant);
                if (obj == null)
                {
                    return "null instance";
                }
                else if (!NebulaObject.IsInstanceValid(obj))
                {
