...
namespace NebulaPlugins
{
    public static class Main
    {
        ...
        private static readonly Assembly CoreApiAssembly = typeof(global::Nebula.NebulaObject).Assembly;
        ...
        private static unsafe nebula_bool InitializeFromEngine(IntPtr nebulaDllHandle, nebula_bool editorHint,
            ...
            IntPtr unmanagedCallbacks, int unmanagedCallbacksSize)
        {
            try
            {
                ...
                _dllImportResolver = new NebulaDllImportResolver(nebulaDllHandle).OnResolveDllImport;
                ...
                if (_editorHint)
                {
                    _editorApiAssembly = Assembly.Load("NebulaSharpEditor");
        ...
        {
            public unsafe delegate* unmanaged<char*, nebula_string*, nebula_bool> LoadProjectAssemblyCallback;
            public unsafe delegate* unmanaged<char*, IntPtr, int, IntPtr> LoadToolsAssemblyCallback;
            public unsafe delegate* unmanaged<nebula_bool> UnloadProjectPluginCallback;
        }

        [UnmanagedCallersOnly]
        private static unsafe nebula_bool LoadProjectAssembly(char* nAssemblyPath, nebula_string* outLoadedAssemblyPath)
        {
            try
            {
                if (_projectLoadContext != null)
                    return nebula_bool.True; // Already loaded
            ...
            IntPtr unmanagedCallbacks, int unmanagedCallbacksSize)
        {
            try
            {
                ...
                if (_editorApiAssembly == null)
                    throw new InvalidOperationException("The Nebula editor API assembly is not loaded.");

                var (assembly, _) = LoadPlugin(assemblyPath, isCollectible: false);

                NativeLibrary.SetDllImportResolver(assembly, _dllImportResolver!);

                var method = assembly.GetType("NebulaTools.NebulaSharpEditor")?
                    .GetMethod("InternalCreateInstance",
                        BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public);

                if (method == null)
                {
                    throw new MissingMethodException("NebulaTools.NebulaSharpEditor",
        ...
        [UnmanagedCallersOnly]
        private static nebula_bool UnloadProjectPlugin()
        {
            try
            {
                return UnloadPlugin(ref _projectLoadContext).ToNebulaBool();
            }
            catch (Exception e)
            {
                Console.Error.WriteLine(e);
        ...
        private static bool UnloadPlugin(ref PluginLoadContextWrapper? pluginLoadContext)
        {
            try
            {
                ...
                while (pluginLoadContext.IsAlive)
                {
                    ...
                    if (!takingTooLong && elapsedTimeMs >= 200)
                    {
                        ...
                        // TODO: How to log from NebulaPlugins? (delegate pointer?)
            ...
            {
                // TODO: How to log exceptions from NebulaPlugins? (delegate pointer?)
                Console.Error.WriteLine(e);
                return false;
            }
        }
