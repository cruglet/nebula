...
var file_entries: Dictionary = {} 

## Parse a WBFS disk image to extract an ISO image. [br]
## - Returns and instance of ISO
func parse_wbfs(wbfs: WBFS) -> ISO:
	var iso_image: ISO = ISO.new()
	...
	file_entries = parse_iso_filesystem_from_wbfs(wbfs, filesystem_offset)
...
func get_decrypted_data(offset: int, size: int, wbfs: WBFS) -> PackedByteArray:
