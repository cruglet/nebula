"""
Load this file to your GDB session to enable pretty-printing of some Nebula C++ types.

GDB command: `source misc/utility/nebula_gdb_pretty_print.py`.
...
...
    {
        "description": "Load custom pretty-printers for Nebula types.",
        "text": "source ${workspaceFolder}/misc/utility/nebula_gdb_pretty_print.py"
    }
]
```
...
# Printer for Nebula StringName variables.
class NebulaStringNamePrinter:
...
# Printer for Nebula String variables.
class NebulaStringPrinter:
    def __init__(self, value):
        self.value = value
...
# Printer for Nebula Vector variables.
class NebulaVectorPrinter:
    def __init__(self, value):
        self.value = value
...
def lookup_pretty_printer(value):
    if value.type.name == "StringName":
        return NebulaStringNamePrinter(value)
    if value.type.name == "String":
        return NebulaStringPrinter(value)
    if value.type.name and VECTOR_REGEX.match(value.type.name):
        return NebulaVectorPrinter(value)
    return None
