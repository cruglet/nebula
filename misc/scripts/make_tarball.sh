...
if [ ! -e "version.py" ]; then
  echo "This script should be ran from the root folder of the Nebula repository."
  exit 1
fi
...
while getopts "h?sv:g:" opt; do
  ...
  h|\?)
    ...
    echo
    echo "  -s script friendly file name (nebula.tar.gz)"
    echo "  -v nebula version for file name (e.g. 4.0-stable)"
...
CURDIR=$(pwd)
TMPDIR=$(mktemp -d -t nebula-XXXXXX)
